...
{
    public Day20()
        : base(@"Trench Map", year: 2021, day: 20)
    {
    }

    public override object DetermineStepOneResult(FileReader reader)
    {
        return Iterate(reader, iterateCount: 2);
    }

    public override object DetermineStepTwoResult(FileReader reader)
    {
        return Iterate(reader, iterateCount: 50);
    }
    ...
    private static int Iterate(FileReader reader, int iterateCount)
    {
        ...
        for (var i = 0; i < iterateCount; i++)
        {
            ...
            outsideArea = isOnDictionary[key: 0]
                ? !outsideArea
                : false;
        }
    ...
    private static (Dictionary<int, bool> isOnLookup, HashSet<Point> image, Area area) ParseInput(FileReader reader)
    {
        ...
        var innerReader = new FileReader(string.Join(Environment.NewLine, lines.Skip(count: 1)));
