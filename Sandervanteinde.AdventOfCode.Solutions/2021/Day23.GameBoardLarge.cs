...
internal partial class Day23
{
    public static class GameBoardLarge
    {
        ...
        private static readonly char[] expectedResult = { 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'C', 'C', 'C', 'C', 'D', 'D', 'D', 'D' };

        internal static bool IsCorrect(char[] items)
        {
            return items.AsSpan(start: 7)
        ...
        private static IEnumerable<(char[] board, long score)> Iterator(ReadOnlySpan<char> items)
        {
            ...
            for (var i = 1; i <= 4; i++)
            {
                ...
                {
                    if (column[index: 0] == EMPTY && column[index: 1] == letterForColumn)
                    {
                        continue;
                    }
        ...
        {
            return items.Slice((columnNr * 4) + 3, length: 4);
        }
        ...
        private static bool EmptyOrContainsOnly(in ReadOnlySpan<char> column, char c)
        {
            return (column[index: 0] == c || column[index: 0] == EMPTY)
                ...
                && (column[index: 2] == c || column[index: 2] == EMPTY)
                && (column[index: 3] == c || column[index: 3] == EMPTY);
        }

        public static bool IsColumnFull(ReadOnlySpan<char> items)
        {
            return items[index: 0] != EMPTY
                && items[index: 1] != EMPTY
                && items[index: 2] != EMPTY
                && items[index: 3] != EMPTY;
        }
