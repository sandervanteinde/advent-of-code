...
{
    public Day14()
        : base("Extended Polymerization", year: 2021, day: 14)
    {
    }
    ...
    public override object DetermineStepTwoResult(FileReader reader)
    {
        ...
        {
            var pair = $"{text[j - 1]}{text[j]}";
            var lookupKey = new LookupKey { Key = $"{text[j - 1]}{text[j]}", Index = 1 };
            var thingsToAdd = AddCountsToDictionary(rules, cache, in lookupKey, maxIndex: 40);
    ...
    public Dictionary<char, ulong> AddCountsToDictionary(IReadOnlyDictionary<string, char> rules, Dictionary<LookupKey, Dictionary<char, ulong>> lookup,
        in LookupKey key, int maxIndex)
    {
        ...
        var currentAdded = new[] { new KeyValuePair<char, ulong>(rules[key.Key], value: 1) };
        var resultForIndex = new Dictionary<char, ulong>(currentAdded);
        lookup.Add(key, resultForIndex);

        if (maxIndex > key.Index)
        {
            var leftKey = new LookupKey { Index = key.Index + 1, Key = $"{key.Key[index: 0]}{currentAddedChar}" };
            var rightKey = new LookupKey { Index = key.Index + 1, Key = $"{currentAddedChar}{key.Key[index: 1]}" };

            foreach (var inner in AddCountsToDictionary(rules, lookup, in leftKey, maxIndex)
                         .Concat(AddCountsToDictionary(rules, lookup, in rightKey, maxIndex)))
            {
    ...
    private static (string start, IReadOnlyDictionary<string, char> rules) ParseInput(FileReader reader)
    {
        ...
        foreach (var line in reader.ReadLineByLine())
        {
            ...
            fromToDictionary.Add(
                match.Groups[groupnum: 1].Value, match.Groups[groupnum: 2]
                    .Value[index: 0]
            );
        }
