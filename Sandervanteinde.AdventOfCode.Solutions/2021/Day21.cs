...
internal class Day21 : BaseSolution
{
    ...
    public override object DetermineStepTwoResult(FileReader reader)
    {
        ...
        var (playerOneWins, playerTwoWins) = CalculateWins(new GameState(0, playerOne, playerTwo));
        ...
        )
        {
            ...
            foreach (var (roll, rollValue) in winsForRollValue)
            {
                if (gameState.PlayerTurn == 0) // player one
                {
                    ...
                    var (winsP1, winsP2) = CalculateWins(gameState with
                    {
                        PlayerOneScore = gameState.PlayerOneScore + newPosition,
                ...
                if (gameState.PlayerTurn == 1)
                {
                    ...
                    var (winsP1, winsP2) = CalculateWins(gameState with
                    {
                        PlayerTwoScore = gameState.PlayerTwoScore + newPosition,
    ...
    private (int playerOne, int playerTwo) DetermineStartingPositions(FileReader reader)
    {
        ...
        return (lines.First().Last() - 48, lines.Last().Last() - 48);
    }
