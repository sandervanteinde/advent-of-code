...
{
    public Day02()
        : base("Dive!", year: 2021, day: 2)
    {
    }

    public override object DetermineStepOneResult(FileReader reader)
    {
        var movements = ParseInputToMovements(reader)
            .ToArray();
        var horizontal = 0;
        var vertical = 0;
    ...
    private static IEnumerable<Movement> ParseInputToMovements(FileReader reader)
    {
        ...
        foreach (var match in reader.MatchLineByLine(regex))
        {
            ...
            {
                Amount = int.Parse(match.Groups[groupnum: 2].Value), Direction = Enum.Parse<Direction>(match.Groups[groupnum: 1].Value)
            };
        }
    }
