...
internal class Day03 : BaseSolution
{
    ...
    public Day03()
        : base("Binary Diagnostic", year: 2021, day: 3)
    {
    }

    public override object DetermineStepOneResult(FileReader reader)
    {
        var list = Enumerable.Repeat(element: 0, BITS)
            .Select(_ => new OnOrOff { On = 0, Off = 0 })
            .ToList();
        var bitValues = Enumerable.Repeat(element: 0, BITS)
            .Select((_, index) => (int)Math.Pow(x: 2, index))
            .ToList();
        ...
        var reversed = linkedList.Select(
            i => i == '0'
                ? '1'
                : '0'
        );
        var gammaRate = Convert.ToInt32(new string(linkedList.ToArray()), fromBase: 2);
        var epsilonRate = Convert.ToInt32(new string(reversed.ToArray()), fromBase: 2);
    ...
    public override object DetermineStepTwoResult(FileReader reader)
    {
        ...
        var bitValues = Enumerable.Repeat(element: 0, BITS)
            ...
            .Reverse()
            .ToList();
        var oxygenGeneratorRating = ParseInput(reader)
            .ToList();
        ...
        foreach (var bitValue in bitValues)
        {
            ...
            {
                oxygenGeneratorRating = DetermineNewValue(oxygenGeneratorRating, bitValue, inversed: false);
            }

            if (co2ScrubberRating.Count > 1)
            {
                co2ScrubberRating = DetermineNewValue(co2ScrubberRating, bitValue, inversed: true);
            }
        }

        return oxygenGeneratorRating.Single() * co2ScrubberRating.Single();

        List<int> ExcludingBit(int bit, List<int> list)
        {
            return list.Where(b => (b & bit) == 0)
                .ToList();
        }

        List<int> IncludingBit(int bit, List<int> list)
        {
            return list.Where(b => (b & bit) == bit)
                .ToList();
        }
        ...
        List<int> DetermineNewValue(List<int> oxygenGeneratorRating, int bitValue, bool inversed)
        {
            ...
            if (inversed)
            {
                returnMethod = returnMethod == IncludingBit
                    ? ExcludingBit
                    : IncludingBit;
            }
    ...
    private static IEnumerable<int> ParseInput(FileReader reader)
    {
        ...
        {
            yield return Convert.ToInt32(line, fromBase: 2);
        }
    }
