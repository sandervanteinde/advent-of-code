...
internal class Day08 : BaseSolution
{
    ...
    public Day08()
        : base("Two-Factor Authentication", year: 2016, day: 8)
    {
        _screen = new Screen(width: 50, height: 6);
    }
    ...
    private void ProcessInstructions(FileReader reader)
    {
        foreach (var instruction in reader.ReadLineByLine())
        {
            ...
            {
                _screen.CreateRectangle(int.Parse(rectInstruction.Groups[groupnum: 1].Value), int.Parse(rectInstruction.Groups[groupnum: 2].Value));
                continue;
            }
            ...
            {
                _screen.RotateRow(int.Parse(rotateRowInstruction.Groups[groupnum: 1].Value), int.Parse(rotateRowInstruction.Groups[groupnum: 2].Value));
                continue;
            }
            ...
            {
                _screen.RotateColumn(
                    int.Parse(rotateColumnInstruction.Groups[groupnum: 1].Value), int.Parse(rotateColumnInstruction.Groups[groupnum: 2].Value)
                );
                continue;
            }
