...
{
    public Day02()
        : base("Bathroom Security", year: 2016, day: 2)
    {
    }
    ...
    public override object DetermineStepOneResult(FileReader reader)
    {
        ...
        foreach (var line in reader.ReadLineByLine())
        {
            foreach (var c in line)
            {
                point = c switch
                {
                    'U' => point with { Y = Math.Max(val1: 0, point.Y - 1) },
                    'D' => point with { Y = Math.Min(val1: 2, point.Y + 1) },
                    'L' => point with { X = Math.Max(val1: 0, point.X - 1) },
                    'R' => point with { X = Math.Min(val1: 2, point.X + 1) },
