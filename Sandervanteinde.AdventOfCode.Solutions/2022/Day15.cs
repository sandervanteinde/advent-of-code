...
internal partial class Day15 : BaseSolution
{
    ...
    private IReadOnlyCollection<Sensor> ParseInput(FileReader reader)
    {
        ...
        foreach(var line in reader.ReadLineByLine())
        {
            ...
            if(match is not { Success: true, Groups: [_, { Value: string locationX}, { Value: string locationY}, {  Value: string beaconX }, { Value: string beaconY}] })
            {
            ...
            items.Add(new Sensor
            {
                ...
                Location = new Point
                {
                    X = int.Parse(locationX),
                    Y = int.Parse(locationY)
                }
            });
        }
    ...
    private class Sensor
    {
        ...
        public Point? GetFirstPointOutOfRange(Point point)
        {
            ...
            if (distance <= ManhattanDistance)
            {
                ...
                {
                    X = point.X + (ManhattanDistance - distance) + 1
                };
