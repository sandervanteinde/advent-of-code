...
{
    public Day16()
        : base("Aunt Sue", year: 2015, day: 16)
    {
    }
    ...
    public override object DetermineStepOneResult(FileReader reader)
    {
        ...
        {
            new Possession { Name = "children", Amount = 3 }, new Possession { Name = "cats", Amount = 7 },
            new Possession { Name = "samoyeds", Amount = 2 }, new Possession { Name = "pomeranians", Amount = 3 },
            new Possession { Name = "akitas", Amount = 0 }, new Possession { Name = "vizslas", Amount = 0 },
            new Possession { Name = "goldfish", Amount = 5 }, new Possession { Name = "trees", Amount = 3 }, new Possession { Name = "cars", Amount = 2 },
            new Possession { Name = "perfumes", Amount = 1 }
        }.ToDictionary(x => x.Name, x => x.Amount);

        return ParseSues(reader)
            .First(IsMatch)
            .Number;

        bool IsMatch(Sue sue)
        {
            return sue.Possessions.All(possession => expectedPossessions[possession.Name] == possession.Amount);
        }
    }
    ...
    private static IEnumerable<Sue> ParseSues(FileReader reader)
    {
        ...
        {
            var possessions = line.Groups[groupnum: 2]
                .Value.Split(", ");
            yield return new Sue
            {
                Number = int.Parse(line.Groups[groupnum: 1].Value),
                Possessions = possessions
                    .Select(
                        x =>
                        {
                            var split = x.Split(": ");
                            return new Possession { Name = split[0], Amount = int.Parse(split[1]) };
                        }
                    )
                    .ToList()
                    .AsReadOnly()
