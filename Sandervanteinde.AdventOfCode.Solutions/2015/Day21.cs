...
{
    public Day21()
        : base("RPG Simulator 20XX", year: 2015, day: 21)
    {
    }
    ...
    private static bool IsDragonSlain(IEnumerable<Item> items, int hitpoints, int damage, int armor)
    {
        ...
        while (playerHealth > 0 && dragonHealth > 0)
        {
            if (playerTurn)
            {
                dragonHealth -= Math.Max(val1: 0, playerDamage - armor);
            }
            else
            {
                playerHealth -= Math.Max(val1: 0, damage - playerArmor);
            }
    ...
    private IEnumerable<(int cost, IEnumerable<Item> items)> EnumeratePossibilities(IList<Weapon> weapons, IList<ArmorItem> armors, IList<Ring> rings)
    {
        ...
        void Add()
        {
            result.AddLast((totalCost, items.ToArray()));
        }
    ...
    private static (IList<Weapon> weapons, IList<ArmorItem> armors, IList<Ring> rings) GetStoreItems()
    {
        ...
        {
            yield return new Weapon { Name = "Dagger", Damage = 4, Cost = 8 };
            yield return new Weapon { Name = "Shortsword", Cost = 10, Damage = 5 };
            yield return new Weapon { Name = "Warhammer", Cost = 25, Damage = 6 };
            yield return new Weapon { Name = "Longsword", Cost = 40, Damage = 7 };
            yield return new Weapon { Name = "Greataxe", Cost = 74, Damage = 8 };
        }

        IEnumerable<ArmorItem> Armors()
        {
            yield return new ArmorItem { Name = "Leather", Cost = 13, Armor = 1 };
            yield return new ArmorItem { Name = "Chainmail", Cost = 31, Armor = 2 };
            yield return new ArmorItem { Name = "Splintmail", Cost = 53, Armor = 3 };
            yield return new ArmorItem { Name = "Bandedmail", Cost = 75, Armor = 4 };
            yield return new ArmorItem { Name = "Platemail", Cost = 102, Armor = 5 };
        }

        IEnumerable<Ring> Rings()
        {
            yield return new Ring { Name = "Damage +1", Cost = 25, Damage = 1 };
            yield return new Ring { Name = "Damage +2", Cost = 50, Damage = 2 };
            yield return new Ring { Name = "Damage +3", Cost = 100, Damage = 3 };

            yield return new Ring { Name = "Defense +1", Cost = 20, Armor = 1 };
            yield return new Ring { Name = "Defense +2", Cost = 40, Armor = 2 };
            yield return new Ring { Name = "Defense +3", Cost = 80, Armor = 3 };
        }
    }
