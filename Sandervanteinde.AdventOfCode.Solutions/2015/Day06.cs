...
{
    public Day06()
        : base("Probably a Fire Hazard", year: 2015, day: 6)
    {
    }
    ...
    public override object DetermineStepOneResult(FileReader reader)
    {
        ...
        for (var x = 0; x <= 999; x++)
        {
            for (var y = 0; y <= 999; y++)
            {
                lights.Add((x * 1000) + y, value: false);
            }
        }
            ...
            foreach (var point in IteratePoints(instruction.From, instruction.Through))
            {
                ...
                switch (instruction.Operation)
                {
                    case LightOperation.TurnOn:
                        ...
                        break;
                    case LightOperation.TurnOff:
                        lights[id] = Math.Max(lights[id] - 1, val2: 0);
                        break;
                    case LightOperation.Toggle:
        ...
        return reader.ReadLineByLine()
            .Select(
                line =>
                {
                    ...
                    {
                        Operation = match.Groups[groupnum: 1].Value switch
                        {
                        ...
                        From = new Point { X = int.Parse(match.Groups[groupnum: 2].Value), Y = int.Parse(match.Groups[groupnum: 3].Value) },
                        Through = new Point { X = int.Parse(match.Groups[groupnum: 4].Value), Y = int.Parse(match.Groups[groupnum: 5].Value) }
                    };
                }
            );
    }
