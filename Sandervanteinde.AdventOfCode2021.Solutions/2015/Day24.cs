...
{
    public Day24()
        : base("It Hangs in the Balance", 2015, 24)
    {
    }

    public override object DetermineStepOneResult(FileReader reader)
    {
        var packageValues = ParsePackageValues(reader).ToArray();
        var totalWeight = packageValues.Sum();
        var packageWeightPerSection = totalWeight / 3;
        var list = new LinkedList<long>(packageValues);
        var combinationsThatMakePackageWeight = CombinationsThatMakeSize(list.First, packageWeightPerSection, new(), 0)
            .ToArray();
    ...
    {
        return reader.ReadLineByLine().Select(long.Parse);
    }
}
